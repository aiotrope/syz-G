...
describe('POST users - creating two user', () => {
  beforeEach(async () => {
    const hashed = await bcrypt.hash('testPass34=', 10)

    const user1 = new User({
      username: 'agentX1',
      email: 'testemail1@zyz.io',
      hashedPassword: hashed,
    })

    const user2 = new User({
      username: 'agentX2',
      email: 'testemail2@zyz.io',
      hashedPassword: hashed,
    })
  ...
  test('it can create new user', async () => {
    ...
    const newUser = {
      username: 'agentX3',
      email: 'testemail3@zyz.io',
      password: 'testPass55#',
      confirm: 'testPass55#',
    }

    const postUser = await request(app).post('/api/user/signup').send(newUser)
    expect(postUser.type).toEqual('application/json')
    expect(postUser.body.user.email).toBe(newUser.email)
    expect(postUser.body.user.username).toBe(newUser.username)
    expect(postUser.body.user).toHaveProperty('posts')
    expect(postUser.body.user).toHaveProperty('comments')
  ...
  /* test('it checks for duplicate username as error', async () => {
    ...
    const postUser = await request(app).post('/api/user').send(newUser)
    expect(postUser.type).toEqual('application/json')
